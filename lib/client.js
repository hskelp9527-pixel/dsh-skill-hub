...
`;
    ...
    function SkillHubSection({ t, remote, close }) {
      ...
      // Discover: agent filter first, then search; same-prefix families stay
      // grouped and searched members auto-expand.
      ...
      const { groups: entryGroups, standalone: entryStandalone } = groupSkills(visibleEntries);
      ...
      return h("div", { className: "dsk-root" },
        ...
        view === "discover" ? h("section", { className: "dsk-section" },
          ...
          entryGroups.map((group) => {
            const groupKey = `cat:${agentFilter}:${group.prefix}`;
            ...
            return h(CatalogGroupCard, {
              ...
              t,
              pending,
              busyKey: `group:${agentFilter}:${group.prefix}`,
              busy,
          ...
          entryStandalone.map((entry) => h(CatalogCard, {
            key: `${entry.installs[0]?.sourceId}:${entry.dirName}`,
            entry,
            t,
            busy,
    ...
    const zh = {
      ...
      discoverIntro: "已发现 {agents} 个 Agent、{skills} 个技能（跨端重复已合并 {merged} 个安装，去重后每技能只显示一张卡片），其中 {unloaded} 个尚未加载到 DSH。选择 Agent 筛选后点击「加载」，即可在输入框「/」菜单中使用。",
      allAgents: "全部 Agent",
    ...
    const en = {
      ...
      discoverIntro: "Found {agents} agents and {skills} skills ({merged} cross-agent installs merged — each skill shows one card). {unloaded} are not loaded into DSH yet. Pick an agent filter and press Load to use them from the \"/\" menu.",
