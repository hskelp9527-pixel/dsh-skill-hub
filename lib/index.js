...
/**
 * Thin RPC surface over SkillHubCore.
 *
 * The Typert gateway derives each method's wire fields from the parameter
 * identifiers in the method source (plain identifiers only — no
 * destructuring, defaults, or rest) and invokes the method positionally.
 * Every parameter below must therefore be named exactly like the field the
 * client sends in `args`, e.g. `remote("importSkill", { sourceId, dirName,
 * mode })`. The gateway-parity test in scripts/smoke-test.mjs fails when a
 * signature and the client payloads drift apart.
 */
class SkillHubService extends TypertRemoteService {
  ...
  importSkill(sourceId, dirName, mode) { return this.core.importSkill({ sourceId, dirName, mode }); }
  importAll(sourceId, mode, dirNames) { return this.core.importAll({ sourceId, mode, dirNames }); }
  importUnloaded(mode, agentId) { return this.core.importUnloaded({ mode, agentId }); }
  removeSkill(dirName) { return this.core.removeSkill({ dirName }); }
  readSkill(dirName) { return this.core.readSkill({ dirName }); }
  saveSkill(dirName, content) { return this.core.saveSkill({ dirName, content }); }
  createSkill(name, description) { return this.core.createSkill({ name, description }); }
}
