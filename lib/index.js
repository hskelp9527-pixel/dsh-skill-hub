...
export const AGENT_DEFINITIONS = [
  {
    id: "claude",
    label: "Claude Code",
    dirs: [join(".claude", "skills"), join(".claude", "skills-src")],
    sourceLabels: ["Claude Code", "Claude Code (skills-src)"],
  },
  ...
  { id: "agents", label: "通用 Agents（~/.agents）", dirs: [join(".agents", "skills")] },
  ...
  for (const agent of AGENT_DEFINITIONS) {
    for (let index = 0; index < agent.dirs.length; index += 1) {
      ...
      sources.push({
        ...
        label: agent.sourceLabels?.[index] ?? agent.label,
        path,
      });
    }
  }
